...
public class TestContinuousIcebergEnumerator {
  ...
  public void testDiscoverSplitWhenNoReaderRegistered() {
    ...
    Map<IcebergSourceSplit, IcebergSourceSplit.Status> pendingSplitsEmpty = enumerator.snapshotState().pendingSplits();
    ...
    Map<IcebergSourceSplit, IcebergSourceSplit.Status> pendingSplits = enumerator.snapshotState().pendingSplits();
    Assert.assertEquals(1, pendingSplits.size());
    Assert.assertTrue(pendingSplits.containsKey(splits.get(0)));
    Assert.assertEquals(IcebergSourceSplit.Status.UNASSIGNED, pendingSplits.get(splits.get(0)));
  }
