...
class SourceUtil {
  ...
  static boolean isLocalityEnabled(Table table, ReadableConfig readableConfig, Boolean exposeLocality) {
    Boolean localityEnabled =
        exposeLocality != null ? exposeLocality :
   ...
   * @param splitCountProvider Split count supplier. As the computation may involve expensive split discover,
   *                           lazy evaluation is performed if inferring parallelism is enabled.
