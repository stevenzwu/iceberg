...
public class TestIcebergSourceSplitReader {
  ...
  public static void beforeClass() throws IOException {
    ...
    recordBatchList = Lists.newArrayListWithCapacity(3);
    dataFileList = Lists.newArrayListWithCapacity(2);
    for (int i = 0; i < 3; ++i) {
    ...
    List<IcebergSourceSplit> splits = FlinkSplitPlanner
        .planIcebergSourceSplits(tableResource.table(), scanContext);
    Assert.assertEquals(1, splits.size());
    Assert.assertEquals(3, splits.get(0).task().files().size());
    icebergSplit = SplitHelpers.sortFilesAsAppendOrder(splits.get(0), dataFileList);
  }
  ...
  private IcebergSourceSplitReader createSplitReader() {
    ...
    return new IcebergSourceSplitReader(
        new RowDataReaderFunction(config, table.schema(), null,
  ...
  public void testFullScan() throws Exception {
    IcebergSourceSplit split = icebergSplit;
    reader.handleSplitsChanges(new SplitsAddition(Arrays.asList(split)));

    RecordsWithSplitIds<RecordAndPosition<RowData>> readBatch0 = reader.fetch();
    List<Row> rowBatch0 = readRows(readBatch0, split.splitId(), 0, 0L);
    TestHelpers.assertRecords(rowBatch0, recordBatchList.get(0), TestFixtures.SCHEMA);

    RecordsWithSplitIds<RecordAndPosition<RowData>> readBatch1
        = reader.fetch();
    List<Row> rowBatch1 = readRows(readBatch1, split.splitId(), 1, 0L);
    TestHelpers.assertRecords(rowBatch1, recordBatchList.get(1), TestFixtures.SCHEMA);

    RecordsWithSplitIds<RecordAndPosition<RowData>> readBatch2 = reader.fetch();
    List<Row> rowBatch2 = readRows(readBatch2, split.splitId(), 2, 0L);
    TestHelpers.assertRecords(rowBatch2, recordBatchList.get(2), TestFixtures.SCHEMA);

    RecordsWithSplitIds<RecordAndPosition<RowData>> finishedBatch = reader.fetch();
  ...
  public void testResumeFromEndOfFirstBatch() throws Exception {
    IcebergSourceSplit split = IcebergSourceSplit.fromCombinedScanTask(icebergSplit.task(), 0, 2L);
    reader.handleSplitsChanges(new SplitsAddition(Arrays.asList(split)));

    RecordsWithSplitIds<RecordAndPosition<RowData>> readBatch1 = reader.fetch();
    List<Row> rowBatch1 = readRows(readBatch1, split.splitId(), 1, 0L);
    TestHelpers.assertRecords(rowBatch1, recordBatchList.get(1), TestFixtures.SCHEMA);
  ...
  public void testResumeFromStartOfSecondBatch() throws Exception {
    IcebergSourceSplit split = IcebergSourceSplit.fromCombinedScanTask(icebergSplit.task(), 1, 0L);
    reader.handleSplitsChanges(new SplitsAddition(Arrays.asList(split)));
    ...
    RecordsWithSplitIds<RecordAndPosition<RowData>> finishedBatch
        = reader.fetch();
    Assert.assertEquals(Sets.newHashSet(split.splitId()), finishedBatch.finishedSplits());
    Assert.assertEquals(null, finishedBatch.nextSplit());
  }
  ...
  public void testResumeFromMiddleOfSecondBatch() throws Exception {
    IcebergSourceSplit split = IcebergSourceSplit.fromCombinedScanTask(icebergSplit.task(), 1, 1L);
    reader.handleSplitsChanges(new SplitsAddition(Arrays.asList(split)));

    RecordsWithSplitIds<RecordAndPosition<RowData>> readBatch1 = reader.fetch();
    List<Row> rowBatch1 = readRows(readBatch1, split.splitId(), 1, 1L);
