...
public abstract class ReaderFunctionTestBase<T> {
  ...
  public void testNoCheckpointedPosition() throws IOException {
    ...
    List<T> actual0 = extractRecordsAndAssertPosition(batch0, recordBatchList.get(0).size(), 0, 0L);
    ...
    List<T> actual1 = extractRecordsAndAssertPosition(batch1, recordBatchList.get(1).size(), 1, 0L);
    ...
    List<T> actual2 = extractRecordsAndAssertPosition(batch2, recordBatchList.get(2).size(), 2, 0L);
  ...
  public void testCheckpointedPositionBeforeFirstFile() throws IOException {
    IcebergSourceSplit split = IcebergSourceSplit.fromCombinedScanTask(icebergSplit.task(), 0, 0L);
    CloseableIterator<RecordsWithSplitIds<RecordAndPosition<T>>> reader = readerFunction().apply(split);
  ...
  public void testCheckpointedPositionMiddleFirstFile() throws IOException {
    IcebergSourceSplit split = IcebergSourceSplit.fromCombinedScanTask(icebergSplit.task(), 0, 1L);
    CloseableIterator<RecordsWithSplitIds<RecordAndPosition<T>>> reader = readerFunction().apply(split);

    RecordsWithSplitIds<RecordAndPosition<T>> batch0 = reader.next();
    List<T> actual0 = extractRecordsAndAssertPosition(batch0, 1L, 0, 1L);
  ...
  public void testCheckpointedPositionAfterFirstFile() throws IOException {
    ...
    RecordsWithSplitIds<RecordAndPosition<T>> batch0 = reader.next();
    List<T> actual1 = extractRecordsAndAssertPosition(batch0, recordBatchList.get(1).size(), 1, 0L);
    assertRecords(recordBatchList.get(1), actual1, TestFixtures.SCHEMA);
    batch0.recycle();
  ...
  public void testCheckpointedPositionMidSecondFile() throws IOException {
    ...
    RecordsWithSplitIds<RecordAndPosition<T>> batch1 = reader.next();
    List<T> actual1 = extractRecordsAndAssertPosition(batch1, 1L, 1, 1L);
