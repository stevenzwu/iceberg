...
class EventTimeAlignmentAssignerState {
  ...
  EventTimeAlignmentAssignerState(Collection<IcebergSourceSplitState> currentState, Clock clock) {
    ...
    currentState.forEach(
        icebergSourceSplitState ->
            splitStateMap.put(
                icebergSourceSplitState.split().splitId(),
  ...
  public synchronized EventTimeAlignmentAssignerState addSplits(
      ...
      splits.forEach(
          split ->
              Preconditions.checkArgument(
                  splitStateMap.get(split.splitId()).assignTo(hostName)
                      == IcebergSourceSplitStatus.UNASSIGNED));

      numAssignedSplits += splits.size();
      updateTs();
      updateListeners(
          splits, IcebergSourceSplitStatus.UNASSIGNED, IcebergSourceSplitStatus.ASSIGNED);
    }

    return this;
  }

  public synchronized EventTimeAlignmentAssignerState unassignSplits(
      Collection<IcebergSourceSplit> splits) {
    if (splits.isEmpty()) {
      return this;
    }
    ...
    List<Tuple2<IcebergSourceSplitStatus, IcebergSourceSplit>> modifiedSplits =
        splits.stream()
            ...
            .filter(
                t ->
                    t.f0 == IcebergSourceSplitStatus.ASSIGNED
                        || t.f0 == IcebergSourceSplitStatus.COMPLETED)
            .collect(Collectors.toList());

    List<IcebergSourceSplit> previousAssignedSplits =
        modifiedSplits.stream()
    ...
    updateListeners(
        previousAssignedSplits,
        IcebergSourceSplitStatus.ASSIGNED,
        IcebergSourceSplitStatus.UNASSIGNED);
    updateListeners(
        previousCompletedSplits,
  ...
  public synchronized EventTimeAlignmentAssignerState completeSplits(Collection<String> splitIds) {
    List<IcebergSourceSplit> completedSplitIds =
        splitIds.stream()
            .filter(
                splitId ->
                    splitStateMap.get(splitId).complete() == IcebergSourceSplitStatus.ASSIGNED)
    ...
    if (!completedSplitIds.isEmpty()) {
      ...
      updateTs();
      updateListeners(
          completedSplitIds, IcebergSourceSplitStatus.ASSIGNED, IcebergSourceSplitStatus.COMPLETED);
  ...
  private void updateListenersOnTerminalCondition() {
    listeners.keySet().forEach(StateChangeListener::onNoMoreStatusChanges);
  }

  @VisibleForTesting
  synchronized Collection<IcebergSourceSplit> getUnassignedSplits() {
    return splitStateMap.values().stream()
        .filter(SplitState::isUnassigned)
        .map(SplitState::getSplit)
  ...
  public synchronized Collection<IcebergSourceSplitState> snapshotState() {
    return splitStateMap.values().stream()
        ...
        .map(
            splitState ->
                new IcebergSourceSplitState(splitState.getSplit(), splitState.getStatus()))
        .collect(Collectors.toList());
  }
  ...
  private void updateListeners(
      Collection<IcebergSourceSplit> splits,
      IcebergSourceSplitStatus oldStatus,
      IcebergSourceSplitStatus newStatus) {
    if (!splits.isEmpty()) {
      switch (newStatus) {
        case UNASSIGNED:
