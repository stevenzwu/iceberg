...
class EventTimeAlignmentAssigner implements SplitAssigner, WatermarkTracker.Listener {
  ...
  EventTimeAlignmentAssigner(
    ...
    this.unassignedSplitsMaintainer =
        new UnassignedSplitsMaintainer(
            new AscendingTimestampSplitComparator(timestampAssigner), this.assignerState);
    this.watermarkUpdater =
        new WatermarkUpdater(watermarkTracker, timestampAssigner, this.assignerState);
  }
  ...
  /**
   * For now, we just simply complete the available future. Let enumerator to try {@link
   * #getNext(String)} again and see if there is any assignable split.
   */
