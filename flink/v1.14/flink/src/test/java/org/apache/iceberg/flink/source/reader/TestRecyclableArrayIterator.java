...
public class TestRecyclableArrayIterator {
  ...
  public void testEmptyConstruction() {
    // dummy recycler
    RecyclableArrayIterator<Object> iter = new RecyclableArrayIterator<>(
  ...
  public void testGetElements() {
    ...
    int initialOffset = 3;
    long initialSkipCount = 17;

    // dummy recycler
    RecyclableArrayIterator<String> iter = new RecyclableArrayIterator<>(
        ignored -> System.currentTimeMillis(), elements, elements.length, initialOffset, initialSkipCount);

    for (int i = 0; i < elements.length; i++) {
      RecordAndPosition<String> recAndPos = iter.next();
  ...
  public void testArraySubRange() {
    // dummy recycler
    RecyclableArrayIterator<String> iter = new RecyclableArrayIterator<>(ignored -> System.currentTimeMillis(),
  ...
  public void testRecycler() {
    ...
    RecyclableArrayIterator<String> iter = new RecyclableArrayIterator<>(ignored -> recycled.set(true));
    iter.close();
